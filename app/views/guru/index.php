<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Data Guru BK</h1>
        <p class="text-slate-600">Kelola daftar guru bimbingan konseling.</p>
    </div>
    <button onclick="document.getElementById('modal-guru').classList.remove('hidden')" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition">
        ...
        <tbody class="bg-white divide-y divide-slate-200">
            ...
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    <a href="<?= BASEURL; ?>/guru/hapus/<?= $g['id']; ?>" class="text-red-600 hover:text-red-900" onclick="return confirm('Hapus guru ini?')">Hapus</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
...
<!-- Modal -->
<div id="modal-guru" class="fixed inset-0 bg-slate-900 bg-opacity-50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-lg w-full max-w-md p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold">Tambah Guru BK</h3>
            <button onclick="document.getElementById('modal-guru').classList.add('hidden')" class="text-slate-400 hover:text-slate-600">
        ...
        <form action="<?= BASEURL; ?>/guru/tambah" method="POST">
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="document.getElementById('modal-guru').classList.add('hidden')" class="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-md">Batal</button>
